...
// gyro 라우터
router.post(['/survey', '/result'], async (req, res) => {
  try {
    const { id } = req.body;
    console.log(id);
    ...
    const userData = await queryDatabase(user_Info, [id]);
    const gyroRfData = await queryDatabase(gyro_rf_Info, [id]);
    const gyroLfData = await queryDatabase(gyro_lf_Info, [id]);
    const fsrRfData = await queryDatabase(fsr_rf_Info, [id]);
    const fsrLfData = await queryDatabase(fsr_lf_Info, [id]);
    const walkingData = await queryDatabase(walking_Info, [id]);

    if (userData.length > 0 && walkingData.length > 0 && gyroRfData.length > 0 && gyroLfData.length > 0 && fsrRfData.length > 0 && fsrLfData.length > 0) {
      console.log('user, walking, gyro_rf, gyro_lf, fsr_rf, fsr_lf 정보 확인 완료');
      res.json({ userData, walkingData, gyroLfData, gyroRfData, fsrRfData, fsrLfData }); // walkingData 추가
    } else {
